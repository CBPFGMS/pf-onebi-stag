...
const classPrefix = "pfcpmain",
	...
	arcGenerator = d3
		.arc()
		.outerRadius(piesSize / 2)
		.innerRadius(0),
	arcGeneratorRegions = d3.arc().innerRadius(0),
	pieGenerator = d3
		.pie()
		.value(d => d.value)
		.sort((a, b) => b.fundType - a.fundType),
...
function createCountryProfile(
	...
	cerfId = +Object.keys(lists.fundTypesList).find(
		e => lists.fundTypesList[e] === "cerf"
	);
	cbpfId = +Object.keys(lists.fundTypesList).find(
		e => lists.fundTypesList[e] === "cbpf"
	);

	pooledFundsInData.forEach(e =>
		e.funds.sort((a, b) =>
			lists.fundNamesList[a.fund].localeCompare(
				lists.fundNamesList[b.fund]
			)
		)
	);
	...
	if (queryStringObject && queryStringObject.country) {
		...
		if (queryStringObject.fund)
			chartState.selectedFund = queryStringObject.fund;
		drawCountryProfile(
			worldMap,
			rawAllocationsData,
			pooledFundsInData,
	...
	const countries = createListMenu(
		selections,
		lists,
		pooledFundsInData,
		outerDiv,
...
function createListMenu(
	...
	const flatFundsArray = pooledFundsInData
		.flatMap(e => e.funds)
		.sort((a, b) =>
			lists.fundNamesList[a.fund].localeCompare(
				lists.fundNamesList[b.fund]
	...
	const headerContainer = outerDiv
		.append("div")
		.attr("class", "country-pro-main");

	const title = headerContainer.append("h1").html("Country Profile");

	const intro = headerContainer.append("p").html(menuIntroText);

	const innerContainer = outerDiv
		.append("div")
		.attr("class", classPrefix + "innerContainer");

	const alphabetContainer = innerContainer
		.append("div")
		.attr("class", classPrefix + "alphabetContainer");

	const listAndMapContainer = innerContainer
		.append("div")
		.attr("class", classPrefix + "listAndMapContainer");

	const disclaimerContainer = innerContainer
		.append("div")
		.attr("class", classPrefix + "disclaimerContainer");

	const firstAllocationYear = d3.min(flatFundsArray, d =>
		d3.min(d.values, e => e.year)
	);

	disclaimerContainer
		.append("span")
		.html(
			`Based on data from OCHA Grant Manegement System (GMS) since ${firstAllocationYear}`
		);

	const listContainer = listAndMapContainer
		.append("div")
		.attr("class", classPrefix + "listContainer");

	const mapContainer = listAndMapContainer
		.append("div")
		.attr("class", classPrefix + "mapContainer");

	const tableContainer = mapContainer
		.append("div")
		.attr("class", classPrefix + "tableContainer");

	const innerMapContainer = mapContainer
		.append("div")
		.attr("class", classPrefix + "innerMapContainer");

	const mapHeader = innerMapContainer
		.append("div")
	...
	const alphabetButtons = alphabetContainer
		.selectAll(null)
		.data(alphabetData)
		.enter()
		.append("button")
	...
	drawRegionPies(
		piesContainer.piesGroup,
		piesContainer.legendGroup,
		pooledFundsInData,
		colors,
		lists
	);
	...
	alphabetButtons.on("click", (event, datum) => {
		...
		countries.style("display", d =>
			datum === "all" ||
			lists.fundNamesList[d.fund][0].toLowerCase() === datum.toLowerCase()
				? null
				: "none"
		);
	});
...
function createMap(mapData, container) {
	...
	const mapGroup = mapSvg
		...
		.attr(
			"transform",
			`translate(${-(
				(negativeMarginWidth + negativeLeftMargin) *
				mapWidth
			)},${-(negativeMarginHeight * mapHeight)})`
		);

	const piesGroup = mapSvg
		.append("g")
		.attr(
			"transform",
			`translate(${-(
	...
	const legendGroup = mapSvg
		.append("g")
		.attr(
			"transform",
			`translate(${legendPadding},${
				mapHeight - legendPadding - legendHeight
			})`
		);
	...
	mapProjection.fitExtent(
		[
			[0, 0],
			[
				mapWidth + 2 * (negativeMarginWidth * mapWidth),
				mapHeight + 2 * (negativeMarginHeight * mapHeight),
			],
		],
		countryFeatures
	);
	...
	for (let region in regionCentroids) {
		const projected = mapProjection([
			regionCentroids[region].lon,
			regionCentroids[region].lat,
		]);
...
function drawRegionPies(container, legendContainer, data, colors, lists) {
	...
	const pieGroupEnter = pieGroup
		...
		.attr(
			...
			d =>
				"translate(" +
				regionCentroids[d.region].x +
				"," +
				regionCentroids[d.region].y +
				")"
		);
	...
	let slices = pieGroup.selectAll("." + classPrefix + "slice").data(
		d =>
			pieGenerator(
				[
					{
						value: d.fundTypes.find(e => e.fundType === cerfId)
							? d.fundTypes.find(e => e.fundType === cerfId).value
							: 0,
						total: d3.sum(d.fundTypes, e => e.value),
						type: "cerf",
					},
					{
						value: d.fundTypes.find(e => e.fundType === cbpfId)
							? d.fundTypes.find(e => e.fundType === cbpfId).value
							: 0,
						total: d3.sum(d.fundTypes, e => e.value),
						type: "cbpf",
					},
	...
	const slicesRemove = slices.exit().remove();
	...
	let regionNames = pieGroup
		.selectAll(`.${classPrefix}regionNames`)
		.data(d => [d]);

	const regionNamesExit = regionNames.exit().remove();

	const regionNamesEnter = regionNames
		.enter()
		.append("text")
	...
	regionNames
		.attr(
			"y",
			d =>
				radiusScale(d3.sum(d.fundTypes, e => e.value)) +
				regionNamesPadding
		)
		.text(d => d.region);

	let regionValues = pieGroup
		.selectAll(`.${classPrefix}regionValues`)
		.data(d => d.fundTypes);

	const regionValuesExit = regionValues.exit().remove();

	const regionValuesEnter = regionValues
		.enter()
		.append("text")
		.attr("class", classPrefix + "regionValues")
		.style("fill", d =>
			d3
				.color(colors[lists.fundTypesList[d.fundType]])
				.darker(darkerValues)
		)
		.attr("y", (d, i, n) => {
			const parentDatum = d3.select(n[i].parentNode).datum().fundTypes;
			return (
				-radiusScale(d3.sum(parentDatum, e => e.value)) -
				i * regionValuesSpacing -
				regionValuesPadding
			);
		})
		.text(d => lists.fundTypesList[d.fundType] + ": ");

	const regionValuesSpan = regionValuesEnter
		.append("tspan")
...
function drawLegend(container) {
	...
	let backgroundRectangle = container
	...
	let legendSizeGroups = container
	...
	const legendSizeCircles = legendSizeGroupEnter
		...
		.attr(
			"cy",
			d => legendPaddings[0] + radiusScale.range()[1] * 2 - radiusScale(d)
		)
	...
	const legendSizeCirclesText = legendSizeGroupEnter
		...
		.attr(
			"x",
			legendPaddings[3] + radiusScale.range()[1] + legendLineSize + 4
		)
		.attr("y", (d, i) =>
			i === 1
				? legendPaddings[0] +
...
function createRegionsTable(container, data, colors, lists) {
	container.append("span").html("All regions, all-time allocations:");
	...
	const tableRow = table.selectAll(null).data(allData).enter().append("tr");

	const tableCell = tableRow
		.selectAll(null)
		...
		.html((d, i) =>
			i
				? "$" + formatSIFloat(d).replace("G", "B")
				: lists.fundTypesList[d].toUpperCase()
		);
}
...
function drawCountryProfile(
	...
	const mergedYears = Array.from(
		new Set([...yearsSetAllocations, ...yearsSetContributions])
	);
	...
	breadcrumb.secondBreadcrumbSpan.html(
	...
	const dropdownAndButtonsDiv = outerDiv
		.append("div")
		.attr("class", classPrefix + "dropdownAndButtonsDiv");

	const dropdownDiv = dropdownAndButtonsDiv
		.append("div")
		.attr("class", classPrefix + "dropdownDiv");

	const yearsButtonsDiv = dropdownAndButtonsDiv
		.append("div")
		.attr("class", classPrefix + "yearsButtonsDiv");

	const fundsButtonsDiv = dropdownAndButtonsDiv
		.append("div")
		.attr("class", classPrefix + "fundsButtonsDiv");

	const tabsOuterDiv = outerDiv
		.append("div")
		.attr("class", classPrefix + "tabsOuterDiv");

	const tabsDiv = tabsOuterDiv
		.append("div")
		.attr("class", classPrefix + "tabsDiv");

	const chartDiv = outerDiv
		.append("div")
		.attr("class", classPrefix + "chartDiv");

	const tooltipDiv = chartDiv
		.append("div")
		.attr("id", classPrefix + "tooltipDiv")
		.style("display", "none");

	const tooltipDivYears = outerDiv
		.append("div")
	...
	yearsButtons = createYearsButtons(
		yearsButtonsDiv,
		chartState.selectedCountryProfileTab === tabsData[tabsData.length - 1]
			? yearsSetContributions
			: yearsSetAllocations,
		outerDiv,
		tooltipDivYears
	);
	...
	dropdown.list.on("click", (_, d) => {
		...
		updateTopValues(topValues, selections);
		chartDiv
			.selectChildren("div:not(#" + classPrefix + "tooltipDiv)")
			.remove();
		yearsButtons = createYearsButtons(
			yearsButtonsDiv,
			d === tabsData[tabsData.length - 1]
				? yearsSetContributions
				: yearsSetAllocations,
			outerDiv,
			tooltipDivYears
		);
		yearsButtons.on("click.main", (_, d) =>
			setQueryString("year", d, lists)
		);
		setCallFunctions();
		callDrawingFunction();
	});
	...
	tabs.on("click", (event, d) => {
		...
		if (
			...
			chartState.selectedCountryProfileTab.includes("Contributions")
		) {
			yearsButtons = createYearsButtons(
				yearsButtonsDiv,
				d === tabsData[tabsData.length - 1]
		...
		fundsButtons.style("display", e =>
			d === tabsData[tabsData.length - 1] ||
			((d === tabsData[1] || d === tabsData[3]) && e === "cerf/cbpf")
				? "none"
				: null
		);
	...
	function setCallFunctions() {
		if (chartState.selectedCountryProfileTab === tabsData[0])
			tabsCallingFunctions.find(
				d => d.name === tabsData[0]
			).callingFunction = createCountryProfileOverview(
				chartDiv,
				lists,
				colorsObject,
				worldMap,
		...
		if (chartState.selectedCountryProfileTab === tabsData[1])
			tabsCallingFunctions.find(
				d => d.name === tabsData[1]
			).callingFunction = createCountryProfileByPartner(
				chartDiv,
				lists,
				colorsObject,
				tooltipDiv,
		...
		if (chartState.selectedCountryProfileTab === tabsData[2])
			tabsCallingFunctions.find(
				d => d.name === tabsData[2]
			).callingFunction = createCountryProfileBySector(
				chartDiv,
				lists,
				colorsObject,
				tooltipDiv,
		...
		if (chartState.selectedCountryProfileTab === tabsData[3])
			tabsCallingFunctions.find(
				d => d.name === tabsData[3]
			).callingFunction = createCountryProfileByPartnerAndSector(
				chartDiv,
				lists,
				colorsObject,
				tooltipDiv,
		...
		if (chartState.selectedCountryProfileTab === tabsData[4])
			tabsCallingFunctions.find(
				d => d.name === tabsData[3]
			).callingFunction = createCountryProfileContributions(
				chartDiv,
				lists,
				colorsObject,
				tooltipDiv,
	...
	function callDrawingFunction() {
		if (chartState.selectedCountryProfileTab === tabsData[0])
			tabsCallingFunctions
				.find(d => d.name === tabsData[0])
				.callingFunction(
					overviewData,
					overviewAdminLevel1Data,
					resetYear,
		...
		if (chartState.selectedCountryProfileTab === tabsData[1])
			tabsCallingFunctions
				.find(d => d.name === tabsData[1])
				.callingFunction(byPartnerData, true, true);
		if (chartState.selectedCountryProfileTab === tabsData[2])
			tabsCallingFunctions
				.find(d => d.name === tabsData[2])
				.callingFunction(bySectorData, true, true);
		if (chartState.selectedCountryProfileTab === tabsData[3])
			tabsCallingFunctions
				.find(d => d.name === tabsData[3])
				.callingFunction(byPartnerAndSectorData, true, true);
		if (chartState.selectedCountryProfileTab === tabsData[4])
			tabsCallingFunctions
				.find(d => d.name === tabsData[3])
				.callingFunction(contributionsData, true, true);
	}
}
...
function createDropdown(container, pooledFundsInData, lists) {
	...
	const dropdownTitleDiv = dropdownContainer
		.append("div")
		.attr("class", classPrefix + "dropdownTitleDiv");

	const dropdownTitle = dropdownTitleDiv
		.append("div")
		.attr("class", classPrefix + "dropdownTitle")
		.html(selectAnOption);

	const dropdownArrow = dropdownTitleDiv
		.append("div")
		.attr("class", classPrefix + "dropdownArrow");

	dropdownArrow.append("i").attr("class", "fa fa-angle-down");
	...
	const dropdownList = dropdownContainer
		.append("div")
	...
	dropdownContainer.on("mouseleave", () =>
		dropdownContainer.classed("active", d => (d.clicked = false))
	);
...
function createYearsButtons(
	container,
	yearsDataSet,
	outerDiv,
	tooltipDivYears
) {
	d3.select("#pfbihpPlayButton").property("disabled", !yearsDataSet.size);

	container.selectChildren().remove();

	const yearsData = Array.from(yearsDataSet).sort((a, b) => a - b);

	const yearLeftArrow = container
		.append("div")
		.attr("class", classPrefix + "yearLeftArrow")
		.style("cursor", "pointer");

	const yearButtonsContainerDiv = container
		.append("div")
		.attr("class", classPrefix + "yearButtonsContainerDiv");

	const yearButtonsContainer = yearButtonsContainerDiv
		.append("div")
		.attr("class", classPrefix + "yearButtonsContainer");

	const yearRightArrow = container
		.append("div")
		.attr("class", classPrefix + "yearRightArrow")
		.style("opacity", fadeOpacity)
		.style("cursor", "default");

	yearLeftArrow.append("i").attr("class", "fas fa-angle-left");

	yearRightArrow.append("i").attr("class", "fas fa-angle-right");

	const yearsButtons = yearButtonsContainer
		.selectAll(null)
		.data(yearsData)
		.enter()
		.append("button")
	...
	let yearButtonsSize, yearButtonsContainerSize;

	setTimeout(function () {
		yearButtonsSize = ~~yearButtonsContainer.node().scrollWidth;
		yearButtonsContainerSize = ~~yearButtonsContainerDiv
			.node()
		...
		yearButtonsContainer.style(
			"left",
			-1 * (yearButtonsSize - yearButtonsContainerSize) + "px"
		);
	}, duration / 10);

	yearLeftArrow.on("click", () => {
		const thisLeft = parseInt(yearButtonsContainer.style("left"), 10);
		yearRightArrow.style("opacity", 1).style("cursor", "pointer");
		yearButtonsContainer
			.transition()
			.duration(duration)
			.style(
				"left",
				Math.min(thisLeft + yearButtonsContainerSize, 0) + "px"
			)
			.on("end", () => {
	...
	yearRightArrow.on("click", () => {
		const thisLeft = parseInt(yearButtonsContainer.style("left"), 10);
		yearLeftArrow.style("opacity", 1).style("cursor", "pointer");
		yearButtonsContainer
			.transition()
			.duration(duration)
			.style(
				"left",
				Math.max(
					thisLeft - yearButtonsContainerSize,
					-1 * (yearButtonsSize - yearButtonsContainerSize)
				) + "px"
			)
			.on("end", () => {
				if (
					parseInt(yearButtonsContainer.style("left"), 10) ===
					-1 * (yearButtonsSize - yearButtonsContainerSize)
				) {
					yearRightArrow
						.style("opacity", fadeOpacity)
	...
	yearsButtons
		.on("mouseover", (event, d) =>
			mouseoverYears(event, d, tooltipDivYears, outerDiv)
		)
...
function repositionYearsButtons(container) {
	const yearButtonsContainerDiv = container.select(
			`.${classPrefix}yearButtonsContainerDiv`
		),
		yearButtonsContainer = container.select(
			`.${classPrefix}yearButtonsContainer`
		),
...
function createTabs(container, data) {
	const ul = container.append("ul").attr("class", "nav nav-tabs");
...
function processAdminLevel1DataForCountryProfileOverview(rawAdminLevel1Data) {
	...
	rawAdminLevel1Data.forEach(row => {
		if (+row.PooledFundId === chartState.selectedCountryProfile) {
			...
			if (foundYear) {
				const foundAdminLevel1 = foundYear.adminLevel1List.find(
					e =>
						e.AdminLocation1 === row.AdminLocation1 &&
						e.FundType === row.FundType &&
						e.AdminLocation1Latitude.toFixed(6) ===
							row.AdminLocation1Latitude.toFixed(6) &&
						e.AdminLocation1Longitude.toFixed(6) ===
							row.AdminLocation1Longitude.toFixed(6)
				);
				if (foundAdminLevel1) {
					foundAdminLevel1.AdminLocation1Budget +=
						row.AdminLocation1Budget;
				} else {
...
function processDataForCountryProfileByPartner(rawAllocationsData, lists) {
	...
	rawAllocationsData.forEach(row => {
		if (row.PooledFundId === chartState.selectedCountryProfile) {
			if (row.FundId === cbpfId) {
				const foundYearAggregated = data.cbpfAggregated.find(
					e => e.year === row.AllocationYear
				);
				if (foundYearAggregated) {
					const foundPartner = foundYearAggregated.values.find(
						e => e.partner === row.OrganizatinonId
					);
					if (foundPartner) {
	...
	function populate(target, row) {
		...
		if (foundYear) {
			const foundPartner = foundYear.values.find(
				e => e.partner === row.PartnerCode
			);
			if (foundPartner) {
				foundPartner.value += row.ClusterBudget;
	...
	rawAllocationsData.forEach(row => {
		if (row.PooledFundId === chartState.selectedCountryProfile) {
			if (row.FundId === cerfId) {
				const foundYearAndSector = data.find(
					e =>
						e.year === row.AllocationYear &&
						e.sector === row.ClusterId
				);
				if (foundYearAndSector) {
					foundYearAndSector.total += row.ClusterBudget;
					foundYearAndSector.cerf += row.ClusterBudget;
					foundYearAndSector.projectsCerf +=
						(foundYearAndSector.projectsCerf === ""
							? ""
							: separator) + row.ProjList;
					foundYearAndSector.partnersCerf +=
						(foundYearAndSector.partnersCerf === ""
							? ""
							: separator) + row.PartnerCode;
				} else {
					data.push({
						year: row.AllocationYear,
			...
			if (row.FundId === cbpfId) {
				if (
					...
					if (foundYearAndSector) {
						...
						foundYearAndSector.projectsCbpf +=
							(foundYearAndSector.projectsCbpf === ""
								? ""
								: separator) + row.ProjList;
						foundYearAndSector.partnersCbpf +=
							(foundYearAndSector.partnersCbpf === ""
								? ""
								: separator) + row.PartnerCode;
					} else {
...
function processDataForCountryProfileByPartnerAndSector(
	rawAllocationsData,
	lists
) {
	const data = {
	...
	function populate(target, row) {
		const foundYear = target.find(e => e.year === row.AllocationYear);
		if (foundYear) {
			const foundPartnerAndSector = foundYear.values.find(
				e => e.partner === row.PartnerCode && e.sector === row.ClusterId
			);
...
function processDataForCountryProfileContributions(
	rawContributionsData,
	lists
) {
	const data = [];
	...
	rawContributionsData.forEach(row => {
		if (row.PooledFundId === chartState.selectedCountryProfile) {
			...
			if (foundYear) {
				const foundDonor = foundYear.values.find(
...
function pushCbpfOrCerf(obj, row, lists) {
	...
	obj[`type${separator}${row.AllocationSurceId}${separator}total`] +=
		+row.ClusterBudget;
}
...
function createListMenuData(rawAllocationsData, lists) {
	...
	rawAllocationsData.forEach(row => {
		const foundRegion = data.find(
			e => e.region === lists.fundRegionsList[row.PooledFundId]
		);
		if (foundRegion) {
			const foundYearRegion = foundRegion.values.find(
				e => e.year === row.AllocationYear
			);
			if (foundYearRegion) {
			...
			const foundFundTypeRegion = foundRegion.fundTypes.find(
				e => e.fundType === row.FundId
			);
			...
			const foundCountry = foundRegion.funds.find(
				e => e.fund === row.PooledFundId
			);
			if (foundCountry) {
				const foundFundTypeCountry = foundCountry.fundTypes.find(
					e => e.fundType === row.FundId
				);
				...
				const foundYear = foundCountry.values.find(
					e => e.year === row.AllocationYear
				);
				if (foundYear) {
...
function processAllData(
	...
	overviewData = processDataForCountryProfileOverview(
		rawAllocationsData,
		lists
	);
	overviewAdminLevel1Data =
		processAdminLevel1DataForCountryProfileOverview(adminLevel1Data);
	byPartnerData = processDataForCountryProfileByPartner(
		rawAllocationsData,
		lists
	);
	bySectorData = processDataForCountryProfileBySector(
		rawAllocationsData,
		lists
	);
	byPartnerAndSectorData = processDataForCountryProfileByPartnerAndSector(
		rawAllocationsData,
		lists
	);
	contributionsData = processDataForCountryProfileContributions(
		rawContributionsData,
		lists
	);
}
...
function createDisabledOption(dropdownContainer, yearsArray) {
	...
	disabledOption = disabledOption
		...
		.html(
			yearsArray[0] +
				" - " +
				Math.min(yearsArray[yearsArray.length - 1], currentYear)
		);
}
...
function updateTopValues(topValues, selections) {
	...
	selections.allocationsTopFigure
		...
		.textTween((_, i, n) => {
			...
			const currValue =
				n[i].textContent === topValuesNoValue
					? 0
					: reverseFormat(n[i].textContent.split("$")[1]);
			const interpolator = d3.interpolate(
				currValue || 0,
