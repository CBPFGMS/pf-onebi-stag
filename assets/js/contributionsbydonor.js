...
function createContributionsByDonor(selections, colors, lists) {
	...
	const bottomDiv = containerDiv.append("div")
		.attr("class", classPrefix + "bottomDiv");

	const nonMemberStatesContainerDiv = bottomDiv.append("div")
		.attr("class", classPrefix + "nonMemberStatesContainerDiv")
		.style("height", nonMemberStateHeight + "px");

	const legendDiv = bottomDiv.append("div")
	...
	const legendTopDiv = legendDiv.append("div")
	...
	function createSmallMultiple(data, memberType) {
		...
		let totalValue = donorSvg.selectAll("." + classPrefix + "totalValue")
			.data(d => [d]);

		totalValue = totalValue.enter()
			.append("text")
			.attr("class", classPrefix + "totalValue")
			.attr("x", svgPadding[3] + (svgWidth - svgPadding[1] - svgPadding[3]) / 2)
			.attr("y", totalValuePadding)
			.merge(totalValue)
			.text(d => "$" + formatSIFloat(chartState.selectedFund === "cerf/cbpf" ? d.cerf + d.cbpf : d[chartState.selectedFund]).replace("G", "B"));
