...
const buttonsObject = {
	...
	createButtons(
		...
		const snapshotTooltip = selections.chartContainerDiv
			.append("div")
		...
		const snapshotDiv = containerSelection
			.append("div")
		...
		snapshotDiv
			.on("mouseover", () => snapshotContent.style("display", "block"))
		...
		playIcon.on("click", (_, d) => {
			...
			const thisYearArrayAllocations =
			...
			if (d.clicked) {
				selections.buttonsOuterContainer
					.classed("options-btn-panel-fix", true)
				...
				selections.buttonsOuterContainer
					.classed("options-btn-panel-fix", false)
...
function loopYears(yearsArray, selections) {
	if (chartState.selectedChart === "countryProfile") {
		const yearButtons = d3
			.select(".pfcpmainyearsButtonsDiv")
			.selectAll("button")
		...
		chartState.selectedYearCountryProfile = [
			yearsArrayCountryProfile[
				++yearIndex % yearsArrayCountryProfile.length
			],
		];
		d3.select("#pfbihpyearNumberText").text(
			chartState.selectedYearCountryProfile[0]
...
function createContributionsCsv(contributionsData, lists, dotSeparator) {
	...
	contributionsData.forEach(row => {
		const validYear =
			...
			chartState.selectedYear === row.FiscalYear ||
			isNaN(+chartState.selectedYear);
		const validFund =
			chartState.selectedChart === "contributionsByCerfCbpf" ||
			chartState.selectedFund === "cerf/cbpf" ||
			chartState.selectedFund === "total" ||
			(chartState.selectedFund === "cerf" &&
				lists.fundNamesList[row.PooledFundId].toLowerCase() ===
					"cerf") ||
			(chartState.selectedFund === "cbpf" &&
				lists.fundNamesList[row.PooledFundId].toLowerCase() !== "cerf");
		if (validYear && validFund) {
			data.push({
				Year: row.FiscalYear,
...
function createSnapshot(type, fromContextMenu, selections) {
	...
	selections.chartContainerDiv.selectAll("svg").each((_, i, n) => {
	...
	if (
		...
		chartState.selectedCountryProfileTab === "Allocations by Partner"
	) {
		...
		chartState.selectedChart === "countryProfile" &&
		chartState.selectedCountryProfileTab === "Contributions by Donor"
	) {
		d3.select(".pfbicpcontrchartContentCerf")
			.select("img")
			.style("display", "none");
	...
	html2canvas(imageDiv).then(function (canvas) {
		...
		if (fromContextMenu && chartState.currentHoveredElement)
			d3.select(chartState.currentHoveredElement).dispatch("mouseout");
		...
		if (
			chartState.selectedChart === "countryProfile" &&
			chartState.selectedCountryProfileTab ===
...
function downloadSnapshotPng(source) {
	...
	const fileName =
		chartState.selectedChart + "_" + dateFormat(currentDate) + ".png";
...
function downloadSnapshotPdf(source, selections) {
	...
	let pdf, pdfHeight, pdfTextPosition;

	const point = 2.834646,
		sourceDimentions = selections.chartContainerDiv
			.node()
	...
	function createLetterhead() {
		const footer =
			"© OCHA CBPF Section 2019 | For more information, please visit pfbi.unocha.org";
